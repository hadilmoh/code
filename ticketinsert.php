...
if(isset($_POST['save_ticket']))
{
    ...
    Where request_department.request_id = requests.id';
     ...
     if(mysqli_num_rows($query_ru) > 0)
     {
        ...
        if($query_run)
        {
              ...
              if(mysqli_num_rows($q) > 0)
              {
                foreach($q as $t)
                {
                  ...
                  VALUES ('http://localhost/ticketing/view_ticket.php?ticket_id=$id','A new ticket has arrived','$id_u')";
                  mysqli_query($con, $qu);
                }
              }
        }
      ...
      $res = [
        'status' => 200,
        'message' => 'The ticket is success added'
        ];
        echo json_encode($res);
        return;
